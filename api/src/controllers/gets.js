const { Country, TouristicActivities, Activities } = require('../db.js');
...
async function getAll_Query(name, res) {
  if (name) {
    ...
    if (country.length > 0) return res.json(country);
...
async function getActivities(res) {
  let act = await TouristicActivity.findAll({
    where: {},
    attributes: ['id', 'name', 'difficulty', 'duration', 'station'],
    include: Country,
  });
  if (act) return res.json(act);
