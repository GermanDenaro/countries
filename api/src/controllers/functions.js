...
async function getActivities(id) {
  if (id) {
    let act = await Activities.findAll({
      where: { countryId: { [Op.iLike]: `%${id}%` } },
    });
    if (act.length > 0) return act; // devuelve arreglo de actividades(link activity to country) buscado por countryId
  } else {
    let act = await Activities.findAll({});
    if (act) return act;
    return null;
...
async function getAct(id) {
  let activity;
  activity = await TouristActivity.findOne({
    where: { id: id },
